...
test("video cover uses the shared-element swipe-down exit animation", async ({ page }) => {
  ...
  const activeVideo = gallerySharedVideo.locator("video");
  await expect(activeVideo).toHaveJSProperty("muted", true);
  ...
  const openBox = await gallerySharedVideo.boundingBox();
  const openPlayerBox = await gallerySharedVideo.locator('[data-video-player="true"]').boundingBox();
  const openVideoBox = await activeVideo.boundingBox();
  expect(openBox).not.toBeNull();
  expect(openPlayerBox).not.toBeNull();
  expect(openVideoBox).not.toBeNull();
  const galleryBox = await page.locator('[data-testid="gallery-touch-layer"]').boundingBox();
  expect(galleryBox).not.toBeNull();
  expect(openBox.width).toBeGreaterThan(galleryBox.width * 0.8);
  expect(openBox.height).toBeGreaterThan(galleryBox.height * 0.8);
  expect(openPlayerBox.width).toBeGreaterThan(openBox.width * 0.9);
  expect(openPlayerBox.width).toBeLessThan(openBox.width * 0.98);
  expect(openPlayerBox.height).toBeGreaterThan(openBox.height * 0.9);
  expect(openPlayerBox.height).toBeLessThan(openBox.height * 0.98);
  expect(Math.abs((openPlayerBox.x + openPlayerBox.width / 2) - (openBox.x + openBox.width / 2))).toBeLessThan(2);
  expect(Math.abs((openPlayerBox.y + openPlayerBox.height / 2) - (openBox.y + openBox.height / 2))).toBeLessThan(2);
  expect(openVideoBox.width).toBeGreaterThan(100);
  expect(openVideoBox.height).toBeGreaterThan(100);
  ...
  const secondVideo = page.locator('[data-testid="carousel-track"] [data-media-id="video-photo-2"] video');
  await expect(secondVideo).toHaveCount(1);
  await expect(secondVideo).toHaveJSProperty("muted", true);
  await expect.poll(async () => secondVideo.evaluate((video) => video.paused)).toBe(false);
