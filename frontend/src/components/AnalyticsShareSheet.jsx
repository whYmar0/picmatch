  ...
  return (
    <BottomSheet open={open} onClose={onClose} title={t("shareAnalytics")} zIndex={zIndex}>
      <div className="space-y-5 pt-1">
        ...
        {/* URL display + Copy — maximally rounded pill */}
        <div className="flex items-center gap-2 pl-4 pr-1.5 py-1.5
          ...
          {loading || !url ? (
            <div className="flex-1 flex items-center gap-2 text-sm text-gray-400 min-w-0">
              <Loader2 size={14} className="animate-spin" />
              <span>…</span>
            </div>
          ) : (
            <p
              className="flex-1 text-sm font-mono truncate text-gray-700 dark:text-gray-200 select-all min-w-0"
              title={url}
            >
              {truncated(url)}
            </p>
        ...
        {/* Regenerate — the primary action right under the link (in place of
            the old "view results" button). One tap asks for confirmation. */}
        {url && !loading && !errored && !confirmOpen && (
          <motion.button
